...
The `.zip` and `.skill` contain the canonical `windows-no-bullshit-audit/` Agent Skill directory plus the MIT `LICENSE`, and exclude the `tests/` authoring infrastructure. `SHA256SUMS` is generated by the same deterministic packager and covers both standalone artifacts.
...
.github/workflows/release.yml          Windows smoke tests + verified build/draft-release CI
```
...
The build validates the Skill, checks manifest identity/version consistency, runs the package validator, and produces deterministic ZIP/Skill bytes plus `dist/SHA256SUMS`.
